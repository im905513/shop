...
class Product_manage_model extends MY_Model {
	...
	{
		$sql = @"SELECT COUNT(*) AS total_rows FROM mod_product $filter ";
	...
	public function get_total_prod_reply_rows($filter="")
	{
		$sql = @"SELECT COUNT(*) AS total_rows FROM mod_prod_reply $filter ";
	...
	public function get_product_list($dataStart, $dataLen, $filter)
	{
		$sql = @"SELECT 
						a.id,
						a.pro_item_no, 
				...
				LEFT JOIN mod_code as c on a.pro_status=c.code_id ".$filter."
				ORDER BY a.modi_time DESC LIMIT $dataStart, $dataLen";
